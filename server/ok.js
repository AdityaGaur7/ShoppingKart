function extractBrands(dataArray) {
    // Initialize an empty array to store the brands
    const brands = [];

    // Iterate over the array of JSON objects
    dataArray.forEach(data => {
        // Check if the brand property exists and is not already in the brands array
        if (data.brand && !brands.includes(data.brand)) {
            brands.push(data.brand);
        }
    });

    // Create a new JSON object with the brands
    const brandsJson = { brands: brands };
...
const brandsJson = extractBrands(jsonData);
